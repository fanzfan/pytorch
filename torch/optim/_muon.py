...
Muon.__doc__ = (
    ...
    Args:
        {_params_doc}. Note that Muon is an optimizer for matrix-shaped parameters (``ndim >= 2``)
            ...
            :func:`torch.optim.param_groups_for_muon` applies the conventional partition.
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> # Partition params: matrix-shaped (ndim >= 2, not embedding/head) -> Muon,
        >>> # all others (biases, norms, embeddings, head) -> AdamW.
...
def param_groups_for_muon(
    ...
    *,
    ...
    Muon is designed for hidden-layer matrix parameters. Biases, normalization
    scales, token/position embeddings, and the final LM head are conventionally
    trained with a standard optimizer such as AdamW. This helper replicates the
    partition used by Keller Jordan's Muon and by Microsoft Dion.
    ...
    * ``name`` contains any substring in ``exclude_name_patterns``.
    * ``exclude_predicate(name, p)`` returns ``True`` (when provided).
    ...
    for name, p in module.named_parameters():
        ...
        is_other = (
            p.ndim < 2
            or (
                exclude_name_patterns is not None
                and any(pat in name for pat in exclude_name_patterns)
            )
